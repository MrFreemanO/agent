...
fn main() {
    // listen for file changes to restart the build
    ...
    if is_release {
        ...
        // Print current directory and target path information for debugging
        ...
        // If the source directory does not contain the file, it needs to be generated
        if !target_image_path.exists() {
            ...
            if let Ok(metadata) = fs::metadata(&target_image_path) {
                ...
                if metadata.len() < 1000000 { // Assume file should be at least 1MB
        ...
        // Important: ensure file also exists in src-tauri/resources directory
